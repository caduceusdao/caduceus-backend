...
contract OlympusStaking is Ownable {
  ...
  function stake(uint256 _amount, address _recipient) external returns (bool) {
    ...
    IERC20(OHM).safeTransferFrom(msg.sender, address(this), _amount); // Transfer OHM from Staking Helper contract into Staking contract
    ...
    // Transfer sOHM from Staking contract to Warmup contract, gons balance is subtracted from total gons of staking and allocated to warmup contract
  ...
  function claim(address _recipient) public {
    ...
    if (epoch.number >= info.expiry && info.expiry != 0) {
      ...
      IWarmup(warmupContract).retrieve( // Transfer sOHM from from Warmup contract to Staker
        _recipient,
  ...
  function rebase() public {
    if (epoch.endBlock <= block.number) {
      ...
      uint256 balance = contractBalance(); // Total Ohm Staked + totalBonus
      uint256 staked = IsOHM(sOHM).circulatingSupply(); // total sOHM - total sOHM belonging to staking contract (2scOHM - total sOHM)
